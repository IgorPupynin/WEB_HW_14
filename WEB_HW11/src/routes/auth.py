...
async def signup(body: UserModel, background_tasks: BackgroundTasks, request: Request, db: Session = Depends(get_db)):
    ...
    background_tasks.add_task(send_email, new_user.email, new_user.username, str(request.base_url))  # тут мб баг
...
async def login(body: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    if not user.confirmed:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Invalid confirm email')
...
async def refresh_token(credentials: HTTPAuthorizationCredentials = Security(security), db: Session = Depends(get_db)):
    ...
    :param credentials: HTTPAuthorizationCredentials: Pass the token to the function
    :param db: Session: Get the database session
    :return: A dict with the new access token, refresh token and a bearer string
    :doc-author: Trelent
    """
...
async def confirmed_email(token: str, db: Session = Depends(get_db)):
    ...
    The confirmed_email function is used to confirm a user's email address.
        ...
        If it has been confirmed already, we return another error message saying so; otherwise we update their account status
            to 'confirmed' using repository_users' confirmed_email function.

    :param token: str: Get the token from the url
    :param db: Session: Get the database session
    :return: A message that the email is already confirmed or that it has been confirmed
    :doc-author: Trelent
    """
...
async def request_email(body: RequestEmail, background_tasks: BackgroundTasks, request: Request,
    ...
    if user.confirmed:
        return {'message': 'Email is already confirmed'}
    if user: background_tasks.add_task(send_email, user.email, user.username, str(request.base_url))
